...
export type SkyblockProfileMember = {
	...
	jacobs_contest: {
		medals_inv: Record<SkyblockJacobsContestBracket, number>;
		...
		>;
		talked: boolean;
		unique_brackets: Record<SkyblockJacobsContestBracket, SkyblockJacobContestCrop[]>;
		migration: boolean;
		personal_bests: Record<SkyblockJacobContestCrop, number>;
	};
	currencies: {
		coin_purse: number;
		motes_purse: number;
		essence: Record<SkyblockEssence, { current: number }>;
	};
	dungeons: {
		dungeon_types: {
...
export type SkyblockJacobsContestBracket = 'bronze' | 'silver' | 'gold';
